...
var primaryCharMap = map[string]string{
	"ch": "gl", "ll": "jh", "rr": "j", "cu": "q", "a": "o", "b": "v",
	"c": "k", "d": "t", "e": "u", "f": "zth", "g": "j", "h": "p", "i": "a",
	"j": "y", "k": "s", "l": "r", "m": "n", "n": "m", "ñ": "ny", "o": "i",
	"p": "b", "q": "f", "r": "s", "s": "z", "t": "d", "u": "õ", "v": "w",
	"w": "", "x": "cs", "y": "g", "z": "x",
}
...
func TranslateToPejelagarto(text string) (string, error) {
	...
	text = applyPrimeProperty(text, 'o', 'â', false)
	text = applyPrimeProperty(text, 'u', 'ê', true)
	...
	if encoding.ExtinctSpeciesCount > 0 {
		...
		vowels, tildeMap := "aeiou", map[rune]rune{'a': 'á', 'e': 'é', 'i': 'í', 'o': 'ó', 'u': 'ú'}
		for i, c := range text {
...
func TranslateFromPejelagarto(pejelagartoText string) (string, error) {
	...
	text = strings.NewReplacer(
		"..", ".", ",,", ",", "’", ",", "‘", ",", "×", "!", "{", "(", "}", ")", "`p", "p",
		"â", "o", "ê", "u", "á", "a", "é", "e", "í", "i", "ó", "o", "ú", "u", "õ", "u",
		"Á", "A", "É", "E", "Í", "I", "Ó", "O", "Ú", "U",
	).Replace(text)
	...
	// *** FIX ***: Use the new robust mapping function for the reverse map.
	reversePrimaryMap := getReverseMap(primaryCharMap)
	text = applyMapWithTagging(text, reversePrimaryMap)
	...
	for i < len(text) {
		...
		for _, key := range sortedKeys {
			if strings.HasPrefix(text[i:], key) {
				// Found the longest possible match at this position.
				// Replace it and wrap with tags.
		...
		if !matched {
			// No key matched at this position, so just append the character.
			result.WriteByte(text[i])
			i++
		}
	}

	// After the entire string is processed, clean up all the temporary tags.
...
func getReverseMap(m map[string]string) map[string]string {
