...
// TestTranslationWithSpecificInputs tests translation with specific known inputs (unit tests, not fuzz)
// These verify that the translation functions work correctly with deterministic inputs
func TestTranslationWithSpecificInputs(t *testing.T) {
testCases := []struct {
name  string
input string
}{
...
// Test translation to Pejelagarto
pejelagarto := TranslateToPejelagarto(tc.input)

// Test reverse translation
reversed := TranslateFromPejelagarto(pejelagarto)

// Clean timestamps for comparison (as done in fuzz tests)
inputCleanedTemp, _ := removeISO8601timestamp(tc.input)
inputCleaned := removeTimestampSpecialCharacters(inputCleanedTemp)
reversedCleanedTemp, _ := removeISO8601timestamp(reversed)
reversedCleaned := removeTimestampSpecialCharacters(reversedCleanedTemp)

// Verify round-trip consistency (after cleaning timestamps)
if reversedCleaned != inputCleaned {
t.Errorf("Round-trip failed (after timestamp cleaning):\n  Input (cleaned):    %q\n  Pejelagarto:        %q\n  Reversed (cleaned): %q",
inputCleaned, pejelagarto, reversedCleaned)
}
})
}
}

// TestBasicTranslationFunctionality ensures basic translation works
func TestBasicTranslationFunctionality(t *testing.T) {
// Test that translation produces non-empty output for non-empty input
result := TranslateToPejelagarto("hello")
if result == "" {
t.Error("TranslateToPejelagarto returned empty string for 'hello'")
}

// Test that reverse translation works
reversed := TranslateFromPejelagarto(result)
inputCleaned := removeTimestampSpecialCharacters("hello")
reversedCleanedTemp, _ := removeISO8601timestamp(reversed)
reversedCleaned := removeTimestampSpecialCharacters(reversedCleanedTemp)

if reversedCleaned != inputCleaned {
t.Errorf("Basic translation reversibility failed")
}
}

// TestTranslationNonEmptyOutput verifies that translation always produces output
func TestTranslationNonEmptyOutput(t *testing.T) {
inputs := []string{"a", "hello", "test", "123"}

for _, input := range inputs {
t.Run(input, func(t *testing.T) {
result := TranslateToPejelagarto(input)
if len(result) == 0 {
t.Errorf("TranslateToPejelagarto(%q) returned empty string", input)
}
})
}
}
